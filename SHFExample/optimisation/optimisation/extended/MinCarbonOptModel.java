...
public class MinCarbonOptModel extends AbstractOptModel{

	private static final int timeslots = Config.timeslots;
	
	public MinCarbonOptModel(String optID, MyAgent agent) throws IloException {	
		super(optID, agent);		
	}
	
	public MinCarbonOptModel(String optID, MyAgent agent, IloCplex cplex) throws IloException {	
			...
			) throws IloException 
	{
		...
		for (int t=0; t<timeslots; t++)
		{
			...
			//Add the carbon cost of energy consumption
			for(IGenerator generator: generatorsToVarsMap.keySet())
				{
					IloNumVar vars[] = generatorsToVarsMap.get(generator);
					IloNumExpr carbon = cplex.prod(vars[t], generator.getCarbonCost()[t]);
					agentUtil = cplex.sum(agentUtil, cplex.negative(carbon));
				}
